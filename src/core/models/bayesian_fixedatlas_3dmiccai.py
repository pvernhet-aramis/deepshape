...
from src.support.networks.nets_vae_3d import MetamorphicFixedAtlas3d
...
class VariationalMetamorphicFixedAtlas3dExecuter(pl.LightningModule):

    def __init__(self, hparams):
        super(VariationalMetamorphicFixedAtlas3dExecuter, self).__init__()
        ...
        # ----------  FIXED ATLAS TEMPLATE
        it_torch = torch.load(self.hparams.intensities_template_path)
        intensities_template = dataset_train.transform(it_torch.unsqueeze(0))      # (batch, channel, width, height, depth)
        assert len(
            intensities_template.size()) == self.hparams.dimension + 2, "atlas size must be (batch, channel, width, height, depth)"
        assert not torch.isnan(intensities_template).any(), "NaN detected"

        # ---------- MODEL
        self.model = MetamorphicFixedAtlas3d(intensities_template, self.hparams.number_of_time_points,
        ...
        self.model.template_intensities = self.model.template_intensities.cuda(self.hparams.num_gpu)
    ...
    def on_epoch_end(self):
        ...
        if self.trainer.current_epoch == 0 or self.trainer.current_epoch % self.hparams.write_every_epoch == 0:
            ...
            print('>> Saved nifti reconstructions.')
...
if __name__ == '__main__':
    ...
    if args.dataset == 'brats':
        ...
        args.output_dir = os.path.join(HOME_PATH, 'Results/MICCAI', '3dEyes', args.experiment_prefix)
    else:
        args.data_tensor_path = os.path.join(HOME_PATH, 'Data/MICCAI_dataset/3_tensors3d', '0_spheres')
        args.output_dir = os.path.join(HOME_PATH, 'Results/MICCAI', '3dSpheres', args.experiment_prefix)

    # Path to tensor Common Atlas
    args.intensities_template_path = os.path.join(HOME_PATH, 'Data/MICCAI_dataset/3_tensors3d',
                                                  'spgr.pt')
    ...
    args.snapshots_path = os.path.join(args.output_dir, 'VAEwFA_{}'.format(args.model_signature))
    ...
    VAE_metamorphicFixed = VariationalMetamorphicFixedAtlas3dExecuter(args)
    ...
    trainer.fit(VAE_metamorphicFixed)
