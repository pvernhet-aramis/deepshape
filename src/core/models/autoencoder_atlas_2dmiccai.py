...
from src.support.networks.nets_vae_2d import AutoencoderAtlas2d
...
class VariationalAutoencoderAtlas2dExecuter(pl.LightningModule):

    def __init__(self, hparams):
        super(VariationalAutoencoderAtlas2dExecuter, self).__init__()
        ...
        # ---------- MODEL
        self.model = AutoencoderAtlas2d(intensities_template, self.hparams.number_of_time_points,
                                        self.hparams.downsampling_data, self.hparams.downsampling_grid,
                                        self.hparams.latent_dimension__a,
                                        self.hparams.kernel_width__a,
    ...
    def training_step(self, batch, batch_idx):
        ...
        means__a, log_variances__a = self.model.encode(batch_target_intensities)
        log_variances__a = torch.clamp(log_variances__a, self.hparams.clipvar_min, self.hparams.clipvar_max)
        stds__a = torch.exp(0.5 * log_variances__a)
        ...
        transformed_template = self.model(batch_latent__a)
        ...
        total_loss = (attachment_loss / self.model.noise_variance + kl_loss__a) / bts
    ...
    def validation_step(self, batch, batch_idx):
        ...
        total_loss = (attachment_loss / self.model.noise_variance + kl_loss__a) / (bts * space_size)
...
if __name__ == '__main__':
    ...
    # Model parameters
    parser.add_argument('--latent_dimension__a', type=int, default=10, help='Latent dimension of s.')
    parser.add_argument('--kernel_width__a', type=float, default=5, help='Kernel width s.')
    parser.add_argument('--lambda_square__a', type=float, default=10. ** 2, help='Lambda square s.')
    ...
    args.snapshots_path = os.path.join(args.output_dir, 'Auto_{}'.format(args.model_signature))
    ...
    VAE_autoencoder = VariationalAutoencoderAtlas2dExecuter(args)
